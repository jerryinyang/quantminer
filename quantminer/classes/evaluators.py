...
class BaseClusterEvaluator(ABC):
    ...
    def __init__(self, n_clusters: int):
        ...
        self._returns_data: Optional[pd.Series] = None
    ...
    def evaluate_all_clusters(
        ...
        """
        # Store returns data with proper datetime index
        if returns is None:
            raise ValueError("Returns data must be provided for cluster evaluation")

        # Convert to Series if numpy array
        if not isinstance(returns, pd.Series):
            self._returns_data = pd.Series(
                returns,
                index=pd.date_range(start="2000-01-01", periods=len(returns), freq="D"),
            )
        # Ensure datetime index if Series
        else:
    ...
    def compute_performance(
        ...
        """
        Compute overall performance using selected clusters.

        Args:
            labels: Cluster labels for each data point
            returns: Return Series (already aligned with labels)

        Returns:
            float: Overall performance metric (martin ratio)
        """
        # Create signals from selected clusters
        signals = np.zeros_like(labels, dtype=float)
        signals[np.isin(labels, self.selected_long)] = 1
        signals[np.isin(labels, self.selected_short)] = -1
        ...
        if len(returns) != len(signals):
            # Ensure equal lengths if necessary
        ...
        return float(qt.stats.ulcer_performance_index(strategy_returns))
...
class VectorPatternEvaluator(BaseClusterEvaluator):
    ...
    def evaluate_all_clusters(
        ...
        data: np.ndarray,
        labels: np.ndarray,
        returns: Union[np.ndarray, pd.Series] = None,  # Make returns optional
    ) -> Dict[int, ClusterStats]:
        """
        Override base class method to ignore returns parameter since pattern evaluation
        doesn't use historical returns data.
...
class HoldingPeriodEvaluator(BaseClusterEvaluator):
    ...
    def evaluate_cluster(
        ...
        # Generate signals for both long and short scenarios
        signals_long = np.zeros_like(labels, dtype=float)
        signals_short = np.zeros_like(labels, dtype=float)

        signals_long[cluster_mask] = 1
        signals_short[cluster_mask] = -1

        # Apply holding period to both signal sets
        signals_long = self._apply_holding_period(signals_long)
        signals_short = self._apply_holding_period(signals_short)

        # Calculate returns using stored returns data
        returns_long = self._returns_data[signals_long != 0]
        returns_short = -self._returns_data[
            signals_short != 0
        ]  # Negate for short positions

        # Convert to series for metric calculation
        returns_long = pd.Series(returns_long)
        returns_short = pd.Series(returns_short)

        # Calculate metrics for both directions
        metrics_long = HoldingPeriodMetrics(
            martin_ratio=float(qt.stats.ulcer_performance_index(returns_long)),
            sharpe_ratio=float(qt.stats.sharpe(returns_long)),
            profit_factor=float(qt.stats.profit_factor(returns_long)),
            win_rate=float(qt.stats.win_rate(returns_long)),
            avg_return=float(returns_long.mean()),
            max_drawdown=float(qt.stats.max_drawdown(returns_long)),
            sample_size=len(returns_long),
        )

        metrics_short = HoldingPeriodMetrics(
            martin_ratio=float(qt.stats.ulcer_performance_index(returns_short)),
            sharpe_ratio=float(qt.stats.sharpe(returns_short)),
            profit_factor=float(qt.stats.profit_factor(returns_short)),
            win_rate=float(qt.stats.win_rate(returns_short)),
            avg_return=float(returns_short.mean()),
            max_drawdown=float(qt.stats.max_drawdown(returns_short)),
            sample_size=len(returns_short),
        )

        # Store the better performing direction's metrics
        metrics = (
            metrics_long
            if metrics_long.martin_ratio > metrics_short.martin_ratio
            else metrics_short
        )
        self.cluster_metrics[cluster_id] = metrics

        # Calculate base class statistics using the selected direction's returns
        returns_to_use = (
            returns_long
            if metrics_long.martin_ratio > metrics_short.martin_ratio
            else returns_short
        ...
        return ClusterStats(
            ...
            score_std=float(np.std(returns_to_use)),
            skewness=float(stats.skew(returns_to_use)),
            kurtosis=float(stats.kurtosis(returns_to_use)),
            confidence_interval=tuple(
                stats.t.interval(
                    0.95,
                    len(returns_to_use) - 1,
                    loc=metrics.avg_return,
                    scale=stats.sem(returns_to_use),
                )
            ),
